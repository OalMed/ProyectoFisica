function getUnidadesCompuestas(){
    return ['Rapidez','Aceleración','Presión','Ángulo','Potencia']
}
class Factor{
...
function TipoMedidaHasUnidades(tipo_unidad){ 
    ...
    if(tipo_unidad==='Rapidez' || tipo_unidad=='Aceleración'){
    ...
    else if(tipo_unidad==='Ángulo'){
        factores[1]=getFactoresFor('Tiempo',true)
    }
    else if(tipo_unidad==='Presión'){
        //respecto al N
        ...
        factores[0].push(new Factor('Atmósfera','atm',1/(1.013e5),null,false))
        factores[0].push(new Factor('Milímetro de mercurio','mm Hg',1/133.3,null,false))
        factores[0].push(new Factor('Torr(Torriceli)','torr',1/133.3,null,false))
        // factores[0].push(new Factor('Torr','mm Hg',1/133.3,null,false))

        factores[1]=getFactoresFor('Área',true)
        //ademas regresa los de Fuerza, area(longitud al cuadrado)
    }
    else if(tipo_unidad==='Potencia'){
        factores[0]=getFactoresFor('Energía')
...
function getFactoresFor(sistema,auxiliar,elevado){
    ...
    if(sistema==='Rapidez' || sistema=='Aceleración' ||  sistema==='Presión'|| sistema=='Potencia'){
        return factores
    }
    ...
    if (sistema==='Longitud' || sistema==='Área'){
        // Respecto al metro
        // x mediada == 1 metro
         
        factores.push(new Factor('milímetro','mm',1000,elevacion))
        factores.push(new Factor('centimetro','cm',100,elevacion))
        factores.push(new Factor('metro','m',1,elevacion))
        factores.push(new Factor('kilómetro','km',0.001,elevacion))
        ...
        factores.push(new Factor('Ángström','Å',10e-10,elevacion))
        factores.push(new Factor('milla náutica','A',1/1853.0935690338308,elevacion))
        factores.push(new Factor('año luz','Año Luz',1/(9.461e15),elevacion))

        factores.push(new Factor('Micrómetro de milímetro','μm',Math.pow(10,6),elevacion))
        factores.push(new Factor('Nanómetro','nm',Math.pow(10,9),elevacion))
        // si es de area hay que elevar esa cantidad al cuadrado
    } 
    else if(sistema==='Volumen'){
         
        // respecto al litro
        factores.push(new Factor('centímetro','cm',Math.cbrt(1000),elevacion))
        ...
        factores.push(new Factor('galón','galon',.264,1))
    ...
    else if(sistema==='Energía'){
        // respecto a calorias
        factores.push(new Factor('Caloría','cal',1,1))
        factores.push(new Factor('Jules','J',1/.239,1,true))
        factores.push(new Factor('Ergio','ergs',Math.pow(10,7)/.239,1,false))
        
        factores.push(new Factor('pie-libra','ft*lb',1/(.239*1.356),1,true))

        // factores.push(new Factor('Jules','ft*lb',equiv))
        factores.push(new Factor('Unidad Térmica Británica','Btu',1/252,1,true))
        // pendiente
        factores.push(new Factor('Electrón-voltio','eV',2.611e+19,1,false))
        factores.push(new Factor('Kilovatio-hora ','kWh',1.1622e-6,1,false))
    }
    else if(sistema==='Masa-energía'){
        // respecto a kg
        factores.push(new Factor('Kilogramo','kg',1))
        factores.push(new Factor('Julio','J',8.988*Math.pow(10,16)))
        factores.push(new Factor('Dalton','u',6.022e+26))
        factores.push(new Factor('Mega-electrón voltio','MeV',5.60958616722e+29))
        ...
        factores.push(new Factor('Unidad Términca Británica','Btu',1/.293))
